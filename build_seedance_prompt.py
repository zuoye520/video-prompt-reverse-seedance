...
def build_prompt(topic: str, duration: int, ratio: str, style: str, segments: list) -> str:
    timeline = "\n".join([segment_line(s) for s in segments[:12]])  # 控制长度
...
def main():
    ...
    parser.add_argument("--style", default="电影级武侠风，写实CG", help="风格描述")
    ...
    shot_breakdown = build_shot_breakdown(segments, args.duration)

    final_text = f"""# Seedance 2.0 二创提示词（自动生成）

## 版本A：直接可用主提示词
{main_prompt}

## 版本B：时间戳分镜版（便于微调）
{args.duration}秒，{args.ratio}，24fps，{args.style}。
主题：{args.topic}
